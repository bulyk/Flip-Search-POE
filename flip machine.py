...
def pricer_div(divine_price):
    ...
    response_div = requests.post('https://www.pathofexile.com/api/trade/search/'+ league, headers=headers, json=json_data_card)
    print(response_div)
    try: 
...
def price_item(divine_price, price_card):
    ...
    if 'Divine' not in item:
        ...
        response_item = requests.post('https://www.pathofexile.com/api/trade/search/'+ league, headers=headers, json=json_data_item)
...
######################################
    ...
    ws = wb['trade']
    ws['G2'] = divine_price
    ws.append([card, price_card, int(quantity_div), profit])
    wb.save(fn)
    wb.close()
...
def divine():
    ...
    response_divine = requests.post('https://www.pathofexile.com/api/trade/search/'+ league, headers=headers, json=json_data_divine)
